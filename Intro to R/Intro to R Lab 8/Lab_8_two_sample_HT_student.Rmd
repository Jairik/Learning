...
```{r}
#Testing Normality for different pulses
summary(Diffs)
result1 <- ad.test(Diffs)
print(result1)

#Data is non-normal, so a wilcox test is best. alternative is set to less since we are comparing the differences
wilcox.test(data$Pulse1, data$Pulse2, paired = TRUE, alternative = "greater")

```

```         
Null Hypothesis- The average pulse of the class was equal the first and second time (mu1=mu2)
Alternative Hypothesis- The average pulse of the class was greater the second time (mu1<mu2)

α = .05

The sample size is 92 and the data is not normal, so the Mann-whitney test is most appropriate.
