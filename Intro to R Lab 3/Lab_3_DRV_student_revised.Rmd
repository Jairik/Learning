---
title: "Lab 3: Discrete Random Variables"
author: "kgteller"
date: "2023-05-29"
output: 
  html_document:
    toc: true
    toc_float: true
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE, warning=FALSE,message=FALSE)

library(tidyverse)
library(extraDistr)
```

```{r}
Binom_plotter<-function(n,p,lb,ub){
x<-seq(0,n)
hx<-dbinom(x,n,p)


i<-x>= lb & x<= ub

hxi<-dbinom(x[i],n,p)


plot(x,hx,type='h', xlim=c(0,n),ylim=c(0,max(hx)))



if (lb>=0&lb<=n){
  abline(v=lb,lty=3,col='blue')
}

if (ub>=0&ub<=n){
  abline(v=ub,lty=3,col='blue')
}
par(new=T)
plot(x[i],hxi,type='h', col="red", xlim=c(0,n),ylim=c(0,max(hx)),xlab="",ylab="" )

par(new=F)


area <- pbinom(ub, n, p) - pbinom(lb, n, p) + dbinom(lb,n,p)
result <- paste("P(",lb,"\u2264 X \u2264",ub,") =",
   signif(area, digits=3), "\n",
   "n=", n, " p=", p)
mtext(result,3)

}

```

# Your Name: JJ McCauley

# Discrete Random Variables

Today you will be comparing histograms for a discrete distribution with the histogram of a random sample of size 100 from that discrete distribution. The goal is to see that a histogram of a random sample resembles the probability distribution from which that sample was selected. If the sample sizes are large enough you will see that by looking at the histogram for a random sample from a given distribution, you might be able to identify the distribution from which the sample was taken.

## Some R Background

In the R language for discrete and continuous distributions there are several prefixes for each distribution that allow us to calculate and visualize different distributions. These four pre-fixes are:

r-Which generates a random sample from the given distribution with certain parameters (ie. runif(), rbinom() )

d-generates the probability density function of a given distribution. This can be used to find $P(X=x)$ for a discrete distribution and the value of the PDF for a continous distribution.

p- generates the cumulative density function of a given distribution. This calculates $P(X\leq x)$ for a given distribution.

q- generates the quantile function for a given distribution. This gives the x value such that $P(X \leq x)=q$ for a given distribution.

## Part 1 The discrete Uniform Distribution

Let a and b be integers, a \< b.The random variable X has the discrete uniform distribution on [a,b] if for each integer k such that $a \leq k \leq b$ the $Pr(X=k)=\frac{1}{b-a+1}$. A notation for this distribution is *dU(1, 30).*

To generate a graph of dU(1,30) we can do the following:

```{r}
# define the x-axis
x_uniform<-seq(1,30, length=30)

# calculate the uniform probabilities
y_uniform<- ddunif(x_uniform,min=1,max=30)

# plot the distribution
plot(x_uniform,y_uniform, type='h')

```

To calculate a probability of $P(5< X \leq 7)$ we would o the following:

```{r}
round(pdunif(7,1,30)-pdunif(5,1,30),5)

```

P(X\>5) = 1 - P(X\<=5)

```{r}
1 - pdunif(5,1,30)
```

P(X\>=5) = 1 - P(X\<=4)

```{r}
1 - pdunif(4,1,30)
```

However, in the case of $P(5\leq X\leq 7)$, we should do some adjustments.

```{r}
round(pdunif(7,1,30)-pdunif(4,1,30),5)
```

Now calculate $P(10 < X \leq 18.1)$ and $P(10 \leq X \leq 18.1)$

```{r}

# 10 < X <= 18.1
pdunif(18.1,1,30) - pdunif(10,1,30)


# 10 <= X <= 18.1
pdunif(18.1,1,30) - pdunif(9,1,30)

```

To get a list of random data in R we can do the following:

```{r echo=true}
unifrom_sample<-rdunif(100,1,30)

```

We can then plot the histogram using the hist function:

```{r}
# basic plot
hist(unifrom_sample)
# correct the bining
hist(unifrom_sample, breaks=c(1,6,11,16,21,26,30),xlim=c(1,30))
```

We can also overlay the distribution over the histogram:

```{r}

a<-1
b<-30
x_uniform<-seq(a,b, length=b)

y_uniform<- dunif(x_uniform,min=a,max=b)



hist(unifrom_sample, breaks=c(1,6,11,16,21,26,30),xlim=c(1,30))

for (i in 1:30){
  segments(i,0,i,1/(b-a), lwd=2)
}




```

Now create 2 more random samples of 100, plot their histograms, and overlay the distribution:

```{r}
#Defining a function to easily plot the histogram and overlay the distribution
plotHistogramAndOverlayDistribution <- function(uniform_sample) {
  a<-1
  b<-30
  x_uniform<-seq(a,b, length=b)
  y_uniform<- dunif(x_uniform,min=a,max=b)
  hist(uniform_sample, breaks=c(1,6,11,16,21,26,30),xlim=c(1,30))
  
  for (i in 1:30){
    segments(i,0,i,1/(b-a), lwd=2)
  }
}

#Obtaining two samples of random data
uniform_sample_1<-rdunif(100,1,30)
uniform_sample_2<-rdunif(100,1,30)

#Call the defined function to plot histograms and overlay the distribution
plotHistogramAndOverlayDistribution(uniform_sample_1)
plotHistogramAndOverlayDistribution(uniform_sample_2)

```

```         
```

### Assignment 1

Now Take three samples from the uniform distribution $dU(1,30)$ but this time make the sample size 1000. Create histograms for each sample and overlay the distribution.

```{r}
#Defining a function to easily plot the histogram and overlay the distribution
plotHistogramAndOverlayDistribution <- function(uniform_sample) {
  a<-1
  b<-30
  x_uniform<-seq(a,b, length=b)
  y_uniform<- dunif(x_uniform,min=a,max=b)
  hist(uniform_sample, breaks=c(1,6,11,16,21,26,30),xlim=c(1,30))
  
  for (i in 1:30){
    segments(i,0,i,1/(b-a), lwd=2)
  }
}


#Obtaining a sample of 1000 random data points
uniform_sample_3<-rdunif(1000,1,30)

#Calling the function to create histograms and overlay the distribution
plotHistogramAndOverlayDistribution(uniform_sample_3)
```

```{r}
#Obtaining a sample of 1000 random data points
uniform_sample_4<-rdunif(1000,1,30)

#Calling the function to create histograms and overlay the distribution
plotHistogramAndOverlayDistribution(uniform_sample_4)
```

```{r}
#Obtaining a sample of 1000 random data points
uniform_sample_5<-rdunif(1000,1,30)

#Calling the function to create histograms and overlay the distribution
plotHistogramAndOverlayDistribution(uniform_sample_5)
```

Discuss how the histograms of the samples of size 100 and 1000 compare to the actual distribution of dU(1,30):

```         
Although both of these random samples show similarities to the actual distribution of dU(1,30), the larger sample size of 1000 proved to consistently be a surperior representation to the actual distribution.
```

**End Assignment 1**

## Part II Binomial Distribution

Now suppose the random variable X has the binomial distribution Bin(n, p) with n, the number of trials and p, the probability of success on a given trial.The random variable X counts the number of successes. Suppose n = 36 and p = .35 and we want to visualize the probability of getting between 15 and 22 successes. That is finding $P(15 \leq X \leq 22)$. We can do this using the Binom_plotter function defined above. This function takes in the number of successes trials n, the probability of success p, a lower bound and an upper bound. Here is how we can use that function:

```{r}
Binom_plotter(36,.35,15,22)
```

What if P(X=15)

```{r}
Binom_plotter(36, 0.35,15,15)
```

What if P(15\<X\<=22)

```{r}
Binom_plotter(36,0.35,16,22)
```

What if P(15\<= X \< 22)

```{r}
Binom_plotter(36,0.35,15,21)
```

What if P(X \>= 22)

```{r}
Binom_plotter(36,0.35,22,1000000)
```

What if P(X \> 22)

```{r}
Binom_plotter(36,0.35,23, 100000)
```

### Assignment 2

Now lets look at the binomial distribution for different values of p. we will fix the number of trials n=36 and vary the value of p:

```{r}
n<-36
p<-seq(.1,.9,by=.05)
x<-seq(0,36)
for (i in 1:length(p)){plot(x,dbinom(x,n,p[i]),type="h",main=substitute(paste("n=36 p= ",a),list(a=p[i])))}

```

Describe the distributions as skewed left, symmetric or skewed right for the different values of *p.* For what values of *p* is the distribution approximately symmetric? For what values of p is the distribution skewed right?

```         
When the value of p is within .35-.65, the distribution is approximately symmetric. When p is less than .35, the graph becomes more skewed right. When p is greater than .65, the graph becomes more skewed left.
```

Now lets practice finding some probabilities from the binomial distribution with n=36 and p=.35:

Find $P(X \geq 17)$

```{r}
#Find the probability when X is within 17 and 1000000
Binom_plotter(36,0.35,17,1000000)
```

Find $P(X \geq 17.4)$ and $P(X \geq 17.6)$

```{r}
#Find the probability when X is within 17.4 and 1000000
Binom_plotter(36,0.35,17.4,1000000)

#Find the probability when X is within 17.6 and 1000000
Binom_plotter(36,0.35,17.6,1000000)
```

Is there any difference in the late 2 probabilities? why or why not?

```         
There is no difference between 17.4 and 17.6. This is because a binomial distribution only considers integers, therefore rounding both 17.4 and 17.6 up to the nearest integer (18). This would result in having the same probability.
```

**End Assignment 2**

## Part III Poisson Distribution

Now suppose X has the Poisson distribution. This distribution expresses the probability of a given number of events occurring in a fixed interval of time; e.g. the number Geiger counter clicks heard in 10 seconds; the number $\lambda$is the mean number of events observed in the fixed interval.

In R we can use the prefixes defined above in front of pois to get probabilities and random values from a poisson distribution.

For a Poisson distribution the commands would be:

rpois(n,lambda)-Which generates a random sample from the Poisson distribution

dpois(x,lambda)-generates $P(X=x)$ for the Poisson distribution

ppois(q,lambda)- calculates $P(X\leq q)$ for a Poisson distribution.

qpois(p,lambda)- This gives the x value such that $P(X \leq x)=p$ for a Poisson distribution.

### Assignment 3

suppose $\lambda=2.2$ Find $P(X>3)$ and $P(2.2< X < 5.1)$

```{r}
#Defining Lambda
Lambda <- 2.2
#Declaring Strings to clean up print statements
string_p1 <- "P(x>3): "
string_p2 <- "P(2.2<X<5.1): "

#Getting probability that X>3
p1 <- 1 - ppois(3, lambda)
print(paste(string_p1, p1))

#Getting the probability that 2.2<x<5.1
p2 <- (ppois(5.1, lambda) - ppois(2.2, lambda))
print(paste(string_p2, p2))

#Additional Testing
cat("\n") #Making a new line
print("Now trying .1, 1, 3, 4, 5, 7, 10")
cat("\n")
Lambdas <- c(.1, 1, 2, 3, 4, 5, 7, 10)
for (i in Lambdas) {
  print(paste("-----", i, "-----"))
  #Getting probability that X>3
  p1 <- 1 - ppois(3, i)
  print(paste(string_p1, p1))
  cat("\n")
  
  #Getting the probability that 2.2<x<5.1
  p2 <- (ppois(5.1, i) - ppois(2.2, i))
  print(paste(string_p2, p2))
  cat("\n")
  cat("\n")
}
```

Now experiment with the parameter $\lambda$ like we did for p in the binomial distribution above. for what values of $\lambda$ is the distribution right-skewed?

```{r}
#Declaring a list of Lambdas that we will check
Lambdas <- c(.1, 2, 3, 4, 5, 6, 12, 20, 30, 75, 100, 125, 150)

#Defining a function that, given a lambda, will generate a random sample and plot the points on a histogram
plot_Poisson_Distribution <- function(lambda) {
  x <- rpois(1000, lambda) #Generates random sample of poisson distribution
  
  #Fix an issue with the breaks
  data_range <- range(x)
  breaks <- seq(floor(data_range[1]), ceiling(data_range[2]))
  
  #Plot on a histogram
  hist(x, breaks = breaks, xlim=c(1,50), main = paste("Poisson Distribution (λ =", lambda, ")"), xlab = "Value of X", ylab = "Frequency")
}


#Call the function for each lambda
for (i in Lambdas) {
  plot_Poisson_Distribution(i)
}
```

```         
When looking at the histograms, it appears as if the poisson distribution right-skewed when lambda is smaller, clearly showcasing being right-skewed when testing from 1-20. However, as Lambda becomes larger, the poisson distribution begins to become more symmetric.
```

**End Assignment 3**

## Part IV Matching distributions

### Assignment 4

In the file titled Samples2.csv there are three columns of data that come from three different distribution. the columns are labeled Sample_1, Sample_2, and Sample_3. They each contain data from different distributions. The three possible distributions are dU(0,20), Bin(50,.6), and Pois(10). Match each sample with the distribution it came from using histograms and descriptive statistics.

```{r}

data0 <- read.csv("Samples (1).csv")

hist(data0$Sample_1)
print("Sample 1: ")
summary(data0$Sample_1)

print("Sample 2: ")
hist(data0$Sample_2)
summary(data0$Sample_2)

print("Sample 3: ")
hist(data0$Sample_3)
summary(data0$Sample_3)

# Generate random samples to compare to
uniform_sample <- rdunif(1000, 0, 20)
binomial_sample <- rbinom(1000, 50, 0.6)
poisson_sample <- rpois(1000, lambda = 10)

# Plot histograms for each sample
hist(uniform_sample, main = "Uniform Distribution")
hist(binomial_sample, main = "Binomial Distribution")
hist(poisson_sample, main = "Poisson Distribution")

```

```         
When summarizing the columns and looking at the histograms, deductions can be made. Firstly, sample 3 is skewed right and is more closely bounded (with a max of 10), leaving me to believe that it is a Pois(10). Nextly, sample 1 is dealing in discrete values and has somewhat even quartiles. This would push me to believe that it is a uniform distribution. However, sample 2 is dealing with continuous values, therefore sample 1 must be a binomial distribution. This leaves sample 2 to be a uniform distribution, which is still believeable due to it's quartiles being somewhat similar. 

In Summary
  Sample 1: Bin(50,.6)
  Sample 2: dU(0,20)
  Sample 3: Pois(10)
```

**End Assignment 4**
